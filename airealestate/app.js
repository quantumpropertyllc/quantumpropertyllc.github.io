...
function initMap() {
    ...
    // 1.5 Create custom panes for layered interaction
    map.createPane('heatmapPane');
    map.getPane('heatmapPane').style.zIndex = 350;
    map.getPane('heatmapPane').style.pointerEvents = 'none';

    map.createPane('rezoningPane');
    map.getPane('rezoningPane').style.zIndex = 450;
...
function renderCrimeHeatmap(geoJsonData) {
    ...
    // CRITICAL FIX: Set pointer-events directly on the canvas element
    ...
    const markersLayer = L.geoJSON(geoJsonData, {
        pointToLayer: (feature, latlng) => {
            return L.circleMarker(latlng, {
                ...
                weight: 1,
                fillOpacity: 0.9,
                pane: 'markerPane' // Ensure markers are always on top and interactive
            });
        },
...
function onEachFeature(feature, layer, type) {
    // Handle both click and tap events for interaction
    ...
    // Skip hover effects for markers on mobile to keep it snappy
    const isMarker = type === 'crime' || type === 'buildingPermits' || type === 'transitStations';

    layer.on({
        mouseover: (e) => { if (!isMarker) highlightFeature(e); },
        mouseout: (e) => { if (!isMarker) resetHighlight(e, type); },
